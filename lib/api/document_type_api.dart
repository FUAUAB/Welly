...
class DocumentTypeApi {
  ...
  DocumentTypeApi([ApiClient apiClient])
  ...
  Future<List<DocumentType>> getAllDocumentTypes() async {
    ...
    String path = "/api/DocumentType".replaceAll("{format}", "json");
    ...
    if (response.statusCode >= 400) {
      ...
      return (apiClient.deserialize(response.body, 'List<DocumentType>')
              as List)
          .map((item) => item as DocumentType)
          .toList();
    } else {
      return null;
    }
  ...
  Future<DocumentType> getDocumentById(int id) async {
    ...
    // create path and map variables
    String path = "/api/DocumentType/{Id}"
    ...
    if (response.statusCode >= 400) {
      ...
      return apiClient.deserialize(response.body, 'DocumentType')
          as DocumentType;
    } else {
      return null;
    }
