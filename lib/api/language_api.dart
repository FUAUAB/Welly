...
class LanguageApi {
  ...
  LanguageApi([ApiClient apiClient])
  ...
  Future<List<Language>> getAllLanguages() async {
    ...
    String path = "/api/Language".replaceAll("{format}", "json");
    ...
    if (response.statusCode >= 400) {
      ...
      return (apiClient.deserialize(response.body, 'List<Language>') as List)
  ...
  Future<Language> getLanguageById(int languageId) async {
    ...
    // create path and map variables
    String path = "/api/Language/{languageId}"
    ...
    if (response.statusCode >= 400) {
      ...
      return apiClient.deserialize(response.body, 'Language') as Language;
    } else {
      return null;
    }
  }
  ...
  Future<Language> getLanguageByIso(String ISO) async {
    ...
    // verify required params are set
    if (ISO == null) {
      throw new ApiException(400, "Missing required param: ISO");
    }

    // create path and map variables
    String path = "/api/Language/{ISO}"
        .replaceAll("{format}", "json")
        .replaceAll("{" + "ISO" + "}", ISO.toString());
