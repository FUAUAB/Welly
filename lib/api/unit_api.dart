...
class UnitApi {
  ...
  Future<SalesUnit> getSalesUnitById(int unitId, {int companyId}) async {
    ...
    String path = "/api/Unit/SalesUnit/{unitId}"
    ...
    if (response.statusCode >= 400) {
      ...
      return apiClient.deserialize(response.body, 'SalesUnit') as SalesUnit;
    } else {
      return null;
    }
  }
  ...
  ///
  ///
  Future<List<SalesUnit>> getSalesUnits({int companyId}) async {
    Object postBody = null;

    // verify required params are set

    // create path and map variables
    String path = "/api/Unit/SalesUnit".replaceAll("{format}", "json");
    ...
    if (response.statusCode >= 400) {
      ...
      return (apiClient.deserialize(response.body, 'List<SalesUnit>') as List)
          .map((item) => item as SalesUnit)
          .toList();
    } else {
      return null;
    }
