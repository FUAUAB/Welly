...
class CurrencyApi {
  ...
  CurrencyApi([ApiClient apiClient])
      : apiClient = apiClient ?? defaultApiClient;

  /// Get a list of all available currency codes.
  ///
  ///
  Future<List<Currency>> currencyCodes() async {
    Object postBody = null;

    // verify required params are set

    // create path and map variables
    String path = "/api/Currency/CurrencyCodes".replaceAll("{format}", "json");
    ...
    if (response.statusCode >= 400) {
      ...
      return (apiClient.deserialize(response.body, 'List<Currency>') as List)
          .map((item) => item as Currency)
          .toList();
    } else {
      return null;
    }
  ...
  Future<Currency> currencyCodesById(int currencyId) async {
    ...
    // verify required params are set
    if (currencyId == null) {
      throw new ApiException(400, "Missing required param: currencyId");
    }

    // create path and map variables
    String path = "/api/Currency/{CurrencyId}"
    ...
    if (response.statusCode >= 400) {
      ...
      return apiClient.deserialize(response.body, 'Currency') as Currency;
    } else {
      return null;
    }
  }
